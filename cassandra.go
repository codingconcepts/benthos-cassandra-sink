...
func init() {
	output.RegisterPlugin(
		"cassandra",
		func() interface{} {
			return NewCassandraConfig()
		},
		func(iconf interface{}, mgr types.Manager, logger log.Modular, stats metrics.Type) (types.Output, error) {
			conf, ok := iconf.(*CassandraConfig)
			if !ok {
				return nil, fmt.Errorf("%v (%T) is not a valid cassandraConfig", iconf, iconf)
			}
			c, err := NewCassandra(*conf, logger, stats)
			if err != nil {
				return nil, err
			}
			return output.NewAsyncWriter("cassandra", 1, c, logger, stats)
		})
